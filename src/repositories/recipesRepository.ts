...
export function buildListRecipesQuery(input: ListRecipesInput): { sql: string; params: any[] } {
  ...
  const sql = `SELECT id, title, difficultyScore, preparationTime, description, timePeriod, warning, region, ingredients, detailedMeasurements, preparationSteps, usage, historicalContext, scientificEvidence, randomKey, isPremium, imageLocalPath
FROM recipes
${whereClause}
${orderClause}
LIMIT ? OFFSET ?`;
